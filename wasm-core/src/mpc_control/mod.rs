...
mod tests {
    ...
    fn keeps_first_steer_fixed_and_controls_within_limits() {
        let xref = vec![
            0.0, 0.0, 4.0, 0.0, 1.0, 0.1, 4.0, 0.08, 2.0, 0.35, 4.0, 0.18, 2.8, 0.8, 3.5, 0.3, 3.1, 1.4, 3.0, 0.4, 3.2,
            2.1, 2.5, 0.45,
        ];
        let last_steer = 0.12;
    ...
    fn aligns_vehicle_yaw_to_wrapped_reference() {
        ...
        let result = mpc_control_preview(xref, 0.0, 0.0, 2.5, -std::f64::consts::PI + 0.01, 0.0, 0.1).expect("preview");
    ...
    fn matches_python_fixture_on_straight_reference() {
        ...
        let straight =
            mpc_control_preview(straight_xref, 0.0, 0.0, 3.0, 0.0, 0.0, 0.07).expect("straight fixture preview");
    ...
    fn stays_close_to_python_fixture_on_turning_reference() {
        ...
        let turning =
            mpc_control_preview(turning_xref, 0.1, -0.05, 2.2, 0.12, 0.08, 0.07).expect("turning fixture preview");

        let expected_controls = vec![
            8.750088, 0.08, 5.475858, 0.519823, 2.239496, 0.698132, -0.308206, 0.698132, -1.910972, 0.698131,
        ];
        let expected_states = vec![
            0.1, -0.05, 2.2, 0.12, 0.252893, -0.031564, 2.812506, 0.124939, 0.448233, -0.007031, 3.195816, 0.166107,
            0.668996, 0.029976, 3.352581, 0.228939, 0.897802, 0.083283, 3.331007, 0.29485, 1.121038, 0.151074,
            3.197238, 0.360333,
        ];
