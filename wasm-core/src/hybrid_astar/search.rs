...
pub(crate) fn generate_neighbour(
    ...
    let steps = (ha_config.motion_distance / ha_config.motion_resolution) as usize;

    for _ in 0..steps {
        car.update(car_config, ha_config.motion_resolution);
        if !start_collided && car.check_collision(car_config, obstacle_coordinates.to_vec()) {
            return None;
        }
        trajectory.push([car.x(), car.y(), car.yaw(), direction as f64]);
    }

    let ijk = calc_ijk(car.x(), car.y(), car.yaw(), heuristic, ha_config.yaw_grid_resolution);
    if !heuristic.contains(ijk.0, ijk.1) {
        return None;
    }

    let distance_cost = if direction == 1 {
        ha_config.motion_distance
    } else {
        ha_config.motion_distance * ha_config.backwards_cost
    };
    let switch_direction_cost = if current.direction != 0 && current.direction != direction {
        ha_config.switch_direction_cost
    } else {
        0.0
    };
    let steer_change_cost = ha_config.steer_change_cost * (steer - current.steer).abs();
...
pub(crate) fn generate_rspath(
    ...
    for path in solve_all_paths(
        ...
        car_config.target_min_turning_radius(),
        0.0,
        ha_config.motion_resolution,
    ) {
        if path_collides(&path, car_config, obstacle_coordinates) {
            continue;
        }
...
pub(crate) fn calc_rspath_cost(
    ...
    for segment in path.segments() {
        ...
        distance_cost += if direction == 1 {
            ...
            length * ha_config.backwards_cost
        };
        if last_direction != 0 && direction != last_direction {
            switch_direction_cost += ha_config.switch_direction_cost;
        }
        last_direction = direction;

        let steer = match segment.kind() {
            SegmentKind::Left => car_config.target_max_steer(),
            SegmentKind::Right => -car_config.target_max_steer(),
            SegmentKind::Straight => 0.0,
        };
        steer_change_cost += ha_config.steer_change_cost * (steer - last_steer).abs();
        last_steer = steer;
        steer_cost += ha_config.steer_cost * steer.abs() * length;
    }
